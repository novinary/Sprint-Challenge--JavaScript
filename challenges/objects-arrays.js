...
// What time period did stegosaurus live in?
...
*/
const animalNames = [];
zooAnimals.forEach((currValue) =>{
  animalNames.push(`Name: ${currValue.animal_name}, Scientific: ${currValue.scientific_name}`);
})
...
const lowerCase = zooAnimals.map((currValue) => {
  return currValue.animal_name.toLowerCase();
})
...
*/
const largerPopulation = zooAnimals.filter((currValue) => {
  return currValue.population < 5;
})
...
*/
const populationTotal = zooAnimals.reduce((totalPop, currValue) => {
  return totalPop + currValue.population;
}, 0)
